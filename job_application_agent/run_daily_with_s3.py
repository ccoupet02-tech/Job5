#!/usr/bin/env python3
"""
Script d'exécution quotidienne du système d'automatisation avec upload S3
Recherche les offres, génère les documents, upload vers S3, et envoie le rapport HTML par email
"""
...
from html_reporter_s3 import HTMLReporterS3

# Setup logger
logger = setup_logger(logs_dir=str(LOGS_DIR), debug=DEBUG_MODE)


class DailyJobApplicationRunnerWithS3:
    """Exécute le processus quotidien d'automatisation avec upload S3"""
    
    def __init__(self, test_data_path=None):
        logger.info("=" * 70)
        logger.info("DAILY JOB APPLICATION RUNNER (WITH S3) - Starting")
        ...
        self.html_reporter = HTMLReporterS3()
        self.file_uploader = FileUploader()
        
        logger.info("All components initialized (including FileUploader)")
    ...
    def run(self):
        ...
        try:
            ...
            # 4. Upload des fichiers vers S3
            logger.info("\n📍 STEP 4: Uploading files to S3...")
            results = self._upload_files_to_s3(results)
            ...
            logger.info("\n📍 STEP 6: Sending report via email...")
            ...
            # Résumé final
            logger.info("\n" + "=" * 70)
            logger.info("DAILY JOB APPLICATION RUNNER (WITH S3) - COMPLETED SUCCESSFULLY")
            ...
            logger.info(f"Files uploaded to S3: {successful * 2}")  # CV + Lettre par offre
            logger.info(f"Report sent to: {NOTIFICATION_EMAIL}")
            logger.info("=" * 70 + "\n")
    ...
    def _upload_files_to_s3(self, results: List[ApplicationResult]) -> List[ApplicationResult]:
        """Upload les fichiers générés vers S3 et met à jour les chemins"""
        ...
        for result in results:
            ...
            try:
                # Upload du CV
                if result.cv_path and Path(result.cv_path).exists():
                    cv_url = self.file_uploader.upload_file(result.cv_path)
                    if cv_url:
                        logger.info(f"      ✓ CV uploaded to S3: {cv_url}")
                        result.cv_path = cv_url  # Remplacer le chemin local par l'URL S3
                    else:
                        logger.warning(f"      ✗ Failed to upload CV")
                
                # Upload de la lettre
                if result.cover_letter_path and Path(result.cover_letter_path).exists():
                    letter_url = self.file_uploader.upload_file(result.cover_letter_path)
                    if letter_url:
                        logger.info(f"      ✓ Cover letter uploaded to S3: {letter_url}")
                        result.cover_letter_path = letter_url  # Remplacer le chemin local par l'URL S3
                    else:
                        logger.warning(f"      ✗ Failed to upload cover letter")
...
Les fichiers CV et lettre de motivation sont hébergés sur le cloud et accessibles via les liens du rapport.
...
def main():
    """Point d'entrée"""
    import argparse
    parser = argparse.ArgumentParser(description="Daily Job Application Runner with S3")
    parser.add_argument("--test-data", type=str, help="Path to a JSON file with test job offers to bypass scraping.")
    args = parser.parse_args()

    try:
        runner = DailyJobApplicationRunnerWithS3(test_data_path=args.test_data)
        success = runner.run()
