"""
Générateur de rapport HTML interactif pour les offres d'emploi
"""
...
class HTMLReporter:
    """Génère un rapport HTML interactif avec toutes les offres"""
    ...
    def generate_report(self, results: List[ApplicationResult], filename: str = None) -> str:
        ...
        # Générer le HTML avec les chemins relatifs
        html_content = self._create_html(results, output_path)
    ...
    def _create_html(self, results: List[ApplicationResult], output_path: Path = None) -> str:
        """Crée le contenu HTML du rapport"""
        
        # Fonction pour créer un chemin relatif
        def get_relative_path(file_path: str) -> str:
            """Convertit un chemin absolu en chemin relatif par rapport au dossier output"""
            if not file_path:
                return None
            try:
                file_path_obj = Path(file_path)
                if output_path:
                    return file_path_obj.name  # Utiliser juste le nom du fichier
                return file_path
            except:
                return file_path
        ...
        for i, result in enumerate(results, 1):
            ...
            # Boutons de téléchargement avec chemins relatifs
            cv_rel_path = get_relative_path(result.cv_path)
            letter_rel_path = get_relative_path(result.cover_letter_path)
            
            cv_button = f'<a href="{cv_rel_path}" download style="background-color: #007bff; color: white; padding: 8px 12px; text-decoration: none; border-radius: 3px; margin-right: 5px; display: inline-block; font-size: 12px; cursor: pointer;">📄 CV</a>' if result.cv_path and Path(result.cv_path).exists() else '<span style="color: #999; font-size: 12px;">📄 CV</span>'
            
            letter_button = f'<a href="{letter_rel_path}" download style="background-color: #007bff; color: white; padding: 8px 12px; text-decoration: none; border-radius: 3px; margin-right: 5px; display: inline-block; font-size: 12px; cursor: pointer;">📝 Lettre</a>' if result.cover_letter_path and Path(result.cover_letter_path).exists() else '<span style="color: #999; font-size: 12px;">📝 Lettre</span>'
...
<body>
    <div class="container">
        ...
        <div class="footer">
            ...
            <p>1. ✅ Assurez-vous que le rapport HTML et les fichiers PDF (CV et Lettre) sont dans le même dossier</p>
            <p>2. 📄 Cliquez sur les boutons "📄 CV" et "📝 Lettre" pour télécharger vos documents optimisés</p>
            <p>3. 🔗 Cliquez sur "🔗 Postuler" pour accéder à l'offre d'emploi</p>
            <p>4. 📤 Soumettez votre candidature directement sur le site de l'offre</p>
